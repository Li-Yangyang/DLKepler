...
def incl(rprs, a, srad, duration, P):
    a = 215.0537*a
    incl = np.arccos(np.sqrt((1+rprs)**2.0-(a/srad*\
            np.sin(np.pi*duration/24.0/P))**2.0)/(a/srad)*srad)*180.0/np.pi
    return incl
...
class ParaSampler(object):
    ...
    def __init__(self, smass, srad, duration):
        ...
        smass_pop = np.random.uniform(np.min(self.smass), np.max(self.smass), 1)
        a_pop = ((P_pop/365.0)*smass_pop)**(1.0/3.0)
        rprs_pop = np.random.uniform(0,0.1, 1)
        duration_pop = np.random.uniform(np.min(self.duration), np.percentile(self.duration, 97), 1)
        srad_pop = np.random.uniform(np.min(self.srad), np.percentile(self.srad, 99), 1)
        inc_pop = incl(rprs_pop, a_pop, srad_pop, duration_pop, P_pop)
    
        self.rprs_pop = rprs_pop[0]
        self.a_pop = a_pop[0]
        self.P_pop = P_pop[0]
        self.inc_pop = inc_pop[0]
